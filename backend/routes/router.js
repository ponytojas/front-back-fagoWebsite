...
router.post('/sign-up', userMiddleware.validateRegister, (req, res, next) => {
    ...
    db.query(text, values, (err, result) => {
            if (result.rows.length) {
                console.log("Received a request for a sign-up, there was an error with the username")
                ...
                bcrypt.hash(req.body.password, 10, (err, hash) => {
                    if (err) {
                        console.log("Received a request for a sign-up, there was an error with the hashing")
                        ...
                        db.query(text, values, (err, database_response) => {
                            if (err) {
                                ...
                                console.log("Received a request for a sign-up, added correctly to database")
...
router.post('/login', (req, res, next) => {
    ...
    db.query(text, values, (err, result) => {
            ...
            if (result.rows.length) {
                console.log("Received a request for a login, where there's an user with that username")
                ...
                bcrypt.compare(
                    ...
                    (bErr, bResult) => {
                        ...
                        if (bErr || !bResult) {
                            ...
                            let text_last_login = "UPDATE user_login set last_login = now() where id = $1"
                            let values_last_login = [result.rows[0].id]
                            db.query(text_last_login, values_last_login, (err, result) => {
                                if (err) {
                                    return res.status(400).send({msg: err});
                                } else {
                                    console.log("Received a request for a login, all data was correct")
                ...
                console.log("Received a request for a login, where there's not an user with that username")
